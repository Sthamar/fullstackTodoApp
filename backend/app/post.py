...
@router.post("/",status_code=status.HTTP_201_CREATED, response_model=CreateTask)
...
@router.put("/{id}", response_model=CreateTask)
def update_task_content(update_task: CreateTask, id: int, db: Session = Depends(get_db)):
    ...
    # Retrieve the updated task from the database to return it
